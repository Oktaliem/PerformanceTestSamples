...
class OdooLoginTest extends Simulation {
	...
	val scn = scenario("RecordedSimulation")
		.exec(http("request_00")
			...
			.headers(headers_0)
			.check(status.is(200))
//			.check(css("""input[type="hidden"]""","value").saveAs("token"))
		...
		.exec(http("request_0")
			...
			//.formParam("csrf_token", "ebead21915cdb8e3f16c2f8caa0f91a257adfed8o1578825329")
...
//		.exec(goToLoginPage, loginToAmbience5, browseBarChartDashboard)
//		.inject(rampConcurrentUsers(0) to (ramp_up) during (myRamp seconds), constantConcurrentUsers(user) during (ramp_up seconds)))
//		.protocols(httpProtocol)

	//mvn gatling:test -Dgatling.simulationClass=OdooLoginTest -Dusers=10 -DrampUp=10
